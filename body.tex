...
Multiple teams within Rubin Observatory have been working towards formalizing the logging
processes ahead of operations. These efforts have been characterized by the Observatory
...
stakeholders use cases, the logging team created an interactive jupyter notebook as our
minimum viable product (MVP). We hosted this MVP within the RSP’s Times Square, a service
to publish jupyter notebooks with project and data rights holders\cite{SQR-062}.
...
Square. We understood further \rhl{Do you mean better?} the audiences we were serving and
...
access to data and services run at Rubin Observatory\cite{Phalanx-Docs}. The applications
...
balancing, and obscures
\rhl{do you mean abstracts?} the infrastructure so our deployment is the same whether the Nightly Digest is deployed on our summit infrastructure or on our US Data Facility (USDF).
...
Table and Recharts providing tabular and visualization infrastructure, and ShadCN/Tailwind
CSS supporting a consistent design system. Data retrieved from our back end are
transformed into interactive client-side views including synchronized plots, filtering
interfaces, and timeline-based operational summaries.\rhl{should you provide references to
e.g. TanStack or ShadCN?  You know your audience better than I do}

\subsubsection{Automated Data Sources}

We access control system telemetry from the EFD, a time-series based database backed by
InfluxDB
\cite{SQR-068}. This data source provides detailed data including commands sent to components of the control
system, as well as data streams from sensors throughout the observatory. To access
exposures and their metadata, the Nightly Digest queries the Consolidated Database
(ConsDb) \cite{DMTN-227}. ConsDB is populated per exposure (rather than time series
\rhl{time stamp?}) by summarizing telemetric data added to the EFD over the duration of an
exposure, or from other services including Rapid Analysis and science validation
pipelines. Initial scientific measurements regarding exposures are accessed via ConsDB.
...
Observing Specialists contribute to three key written data sources \rhl{which} the Nightly
Digest injects: the Exposure Log, the Narrative Log, and the Night Log
\cite{Aranda26}. The Exposure Log provides a list of exposures, but is unique \rhl{distinct?} from ConsDB in
...
by science data reduction pipelines. The Narrative Log provides a stream of comments left
...
investigating complex component interactions. Finally, the Night Log is a written summary
of the entire night’s observation period, summarizing the key events and providing a
higher level understanding of the state of the observatory after the night has concluded.
\rhl{Maybe add for a broader audience''?}

Similar to the Exposure Log, where human comments and context are added to generated
exposure content, the Nightly Digest also queries Jira and Zephyr Scale. Observing
Specialists create and update Jira tickets related to aforementioned anomalies, adding
context, error messages, and the state of the system around these incidents. Zephyr Scale
...
operations.  The Nightly Digest augments these summit-based data sources with almanac data
from the python package Astropy and simulated survey scheduling data from the observatory
system performance team.
\rhl{I think I'd add that Zephyr scale is how observing blocks are identified.  Otherwise you wouldn't care
  about it.  It's a bit different from Jira, as there you're providing an interface to the
  night's problems.}
...
\hline
\rule[-1ex]{0pt}{3.5ex} Jira / Zephyr Scale & API Endpoint & Issue Tracking & Fault tracking and explicit test steps \rhl{I don't think we're using Zephyr for fault tracking}\
\hline
\end{tabular}
\end{center}
\end{table}
...
previous observation night progressed
\rhl{or range of nights; clarify here and remove next sentence?}. The Nightly Digest also allows users the
flexibility to visualize metrics covering any date range in the past. The sidebar offers
users control to choose which telescope’s data \rhl{I'd say which telescope''} to focus on
(Simonyi/AuxTel), what observation day (dayobs) range to request data for, and how many
nights of data. The sidebar also includes a navigation menu to further detailed pages
covered in section ~\ref{sec:addpages}.



The main summary page includes summary cards and more detailed applets to answer core
...
\item\textbf{Performance} - To what extent did the atmospheric conditions and system performance meet the
  observatory’s efficiency and quality requirements? \rhl{How do achieve this?  You report
  what we
    observed, but not the extent to which we met requirements}
...
%>>>> use \label inside caption to get Fig. number with \ref{}
   { \label{fig:sidebar}
The Nightly Digest Sidebar contains selectable menus for the telescope, night, and number
of nights. Below these parameters, the selected observation day is displayed. Further
navigation is shown below.}
   \end{figure}
...
Across the top of the Nightly Digest \rhl{I'd include a link to the figure here, and
elsewhere when you want
  the reader to look at something}, we display four summary
cards: Nighttime Exposures, Open Shutter Efficiency, Time Loss, and Jira Tickets. The
Nighttime Exposures card displays the number of exposures, queried from ConsDB, that were
...
predict how many exposures could be taken each night by the observatory systems
performance scheduler team \cite{Naghib19, RTN-016} \rhl{I don't think the team takes the
data...}. Open Shutter Efficiency builds on the number of exposures taken to aggregate
total time exposing on-sky and compares this to the time available, subtracting out any
time impacted by weather. The Time Loss Card collects the time loss reported from the
Narrative Log entries, and reports a total number of hours and distribution between
weather and technical faults.\rhl{This will soon be replaced by numbers from Observatory
  Status. You're describing the past, so maybe this is OK.  Probably too contentious to
  discuss how unreliable Narrative Log down time estimates have proved to be} The Jira
  Tickets card displays the number of tickets
created in the time frame the user has selected, and includes a note for the number that
was updated. When clicking on the Jira card, a dialog box opens displaying a table of
these tickets with notable attributes, including a summary, the current status, and a link
to the \rhl{issue in the} ticketing system.


When a larger observation day range is selected these cards add together data from each
...
displays of data, some \rhl{Only some?} combining multiple data sources. The Observing
Conditions applet and the Exposure Breakdown applet give more insight into the
distribution of what data was taken and the quality of that data. Then, the Night Report
and Time Accounting applet give further context into the Observation Specialists’ summary
of the night and the breakdown of how the time was spent. \rhl{Do we need the previous few
sentences, from the quality of that data''?} Finally the Visit Map applet displays a
planispheric representation of the visit location taken over the observation day selected,
to give a sense of survey progress and coverage.
...
%>>>> use \label inside caption to get Fig. number with \ref{}
   { \label{fig:cards}
Summary cards across the top of the digest display a quick look at the performance of the
observatory across different measures. \rhl{Do you need this figure, as opposed to e.g.
outlining these panels in
  Fig. \ref{fig:digest}?}
}
   \end{figure}
...
The Observing Conditions applet communicates the overall quality of the set of exposures
...
We are also able to see gaps in exposures highlighted in this chronological
representation. The user can zoom in on a selected portion of the plot to see a subset of
data and hover over any of the data points to display highlighted attributes for each
exposure. This applet communicates the time distribution of exposures taken and their
overall quality throughout the night. Users can evaluate many observatory qualities from
this combined information including cloud cover, the thermal conditions of the atmosphere
and dome environment, and image quality of the exposures\rhl{How do I get clouds or
thermal?  I assume you're interpreting the zero
  point and seeing, but going from there to e.g. clouds is rather complicated and I think
  that this is confusing.  I'd just say something about photometric depth and image
  quality}. The interactivity that we provide in this applet connects to both other
  applets on the page, and the zoom in selection holds when navigating to other pages in
  the Nightly Digest for further investigation.
...
The Observing Conditions applet shows a dialog box when hovering over any individual data
point, displaying further attributes for any exposure.}
   \end{figure}
...
Hover over a specific filter to the bottom right of the observing conditions plot to see
only that set of data displayed in the applet.}
   \end{figure}
...
that bar and be directed to a sorted Data Log page displaying a table of all the exposures
taken and filtered by the selected category.  Users can get a sense of what tests were run
and how the visits spanned our science and engineering goals over a night by interacting
with the Exposure Breakdown applet.
\rhl{I'm not sure that this last sentence adds anything}
...
The Night Report applet is the most narrative driven visual on the main page. This applet
...
This Night Report summarizes written logging from the Narrative Log which is available in
the Context Feed Page described in ~\ref{sec:contextfeed}.
\rhl{Does it? Isn't the night log independent of the narrative log entries?}
...
The end-of-the-night summary narrative is displayed in Night Report replicating some of
the observers' formatting.}
   \end{figure}
   
\subsubsection{Time Accounting}

The Time Accounting applet is an evolving visualization to inform observatory metrics that
...
The Time Accounting applet calculates how the available observing time was spent and
...
gathered using the survey scheduling team’s software package  \rhl{Fix the quoting, or use
\texttt{rubin\_nights}} to add slew and overhead detail to exposure data from ConsDB
\cite{RTN-016}. We also use time based data from the EFD including when the observatory
dome opens and closes.
...
further automated systems and information about observatory status as a whole.
\rhl{Maybe say that this will mean that it'll no longer be ND's job to guess about how time is used?}
\rhl{Link to Fig. \ref{fig:obsstatus}?}
...
%>>>> use \label inside caption to get Fig. number with \ref{}
   { \label{fig:obsstatus}
Time Accounting at the observatory is improving and we are updating our visualizations to
reflect new automated Observatory States. Here we see the cumulative time in various
states and when the states change throughout the observation night.
\rhl{I might choose a night without \texttt{UNKNOWN}, not that anyone will notice}
}
   \end{figure}
...
expanded upon in a Visit Map page and is a result of our inspiration from and
...
While the Nightly Digest’s main summary page was our initial focus, we have found that
additional pages support our use cases of debugging and adding context. These pages tie
into our core questions and expand upon our summary page applets while adding complexity
...
in the Context Feed page, our timeline component displays a horizontal area marking events
as vertical lines from left to right. Each of our timelines has a pink selection area so
users may modify the time selected and displayed in the data pages to span multiple
observation days, partial selections across multiple days, to even a single exposure or
event.
...
The Plots page expands on the Observing Conditions applet to satisfy the users’ desires to
easily access exposure metadata and trends across the selected observation day range. This
page offers users the ability to display plots from any exposure attribute from ConsDB,
and by default displays airmass, PSF FWHM, photometric zero points, and sky brightness.
The user may also select a portion of any plot to zoom in on, and the additional plots
displayed synchronously adjust to that new time period.

\rhl{You need the link!  Fig \ref{fig:plots}.  I think you should 
  note that the plotting capabilities are limited, and that users are recommended to use a
  jupyter notebook, possible in conjunction with an AI agent, for serious data
  exploration.}
...
The Data Log page expands on the main summary page’s Exposure Breakdown applet by
providing a table of all exposures within the selected observation day range. The columns
display exposure metadata from ConsDB and comments from the Exposure Log associated with
each exposure. This table is scrollable and the visible columns can be selected and
defined by the user. Any column may also be sorted or grouped by the available values for
...
viewing Narrative Log entries alongside script configurations and errors, users understand
what the Observing Specialists activities over the course of an observation day.
\rhl{I don't think you need describe ROLEX here.  You mentioned it as a precursor in the introduction}
...
Observing Specialists’ written summary in the Night Report applet and the Time Accounting
applet, to add detail to events that occurred during the available observing time and
...
and programmatic actions were taken, their results \rhl{and details of any faults}, and
how the Observing Specialists and observatory components reacted.

\rhl{I think I'd expand on this.  How you can use the context feed to e.g. find out what was going on when
  a fault was logged, what the error was, etc.  I'll send you a screenshot with an error
  as a possible alternative to the one in \ref{fig:contextfeed} [which I only noticed
  \textit{after} writing this].}
...
The Visit Maps page expands on the main summary page’s Visit Map applet \rhl{maybe note
somewhere that the applet is
  really just an anchor, and too small for serious use?}. This visualization was a result
  of our cross disciplinary work with the survey scheduler team who creates these maps to
  ...
  shown: the planned final depth of LSST survey, the moon and sun coordinates, the
...
The Nightly Digest continues to be improved upon as we work with Observing Specialists,
summit support staff, and project scientists to understand their evolving needs as Rubin
Observatory moves through commissioning and into operations. One major feature in
development is the automation of a unified Observatory Status, with input from Observing
Specialists. This new control system feature has been integrated with the Observing
Specialists’ interface LOVE and the scheduling component of the control software. With
this new telemetry data collected from the EFD, the Nightly Digest will improve the way we
report and visualize our Time Accounting applet, Time Loss card, and our in-development
Time Loss page. This information will be automatically time based and serves as another
use of a timeline, specifically that users would like to see on our main summary page.

Timelines have become increasingly popular with our users and we continue to arrange our
visualizations to include them in more pages that would leverage a timeline based display,
as they have recently been added to our Data Log page. We continue to solicit feedback
from our user base and work to integrate and unify the different logging tools that each
team uses, so that Nightly Digest may become a launching pad towards more detailed
debugging tools. Our future improvements prioritize user experience and feedback towards
making the dashboard more performant and including AuxTel’s data in all pages.

\rhl{I think I'd mention the possibility of generating reports from the information summarised in the ND.
  I'm pretty sure that that was one of the major logging prioritied, and shouldn't be hard
  to implement if given helpful input on a minimum useful product.}
...
\rhl{How does this relate to Fig. \ref{fig:obsstatus}?  It's confusing to have one polished figure
  and one sketch}
}
   \end{figure}
...
%>>>> use \label inside caption to get Fig. number with \ref{}
   { \label{fig:barside}
Digitally created models of each applet are created to visualize the color scheme before
...
\rhl{I think you should say something about plans for making ND more widely available, or, contrariwise, that
  it is intended to remain a project-internal tool.  It's tricky, but you have to say
  something, even if it's fluffy and noncommittal.}
