...
formalizing the logging processes ahead of operations. These efforts have been characterized by the Observatory
...
Environment (LOVE) as well as a view their written logging through OLE\cite{Aranda26}. The
Telescope and Site software team had a view into the Observatory Control System (OCS) and how
each component was running through each application's logs. Sub-system owners had unique
chronograf dashboards (web application to visualize and query data from an InfluxDB instance)
measuring logs and telemetry for their systems. Remote staff working off the summit created
interactive Jupyter Notebooks run in Times Square in the Rubin Science Platform (RSP).
Each technical group had a slightly different focus and measurement of success,
created their own daily/weekly/monthly summary reports, and monitored different slack channels,
or used a mixture of all the methods above to evaluate observing performance.
...
demonstrated in the Observing Specialist driven data sources. The
Observing Specialists contribute to three key written data sources which the Nightly
Digest ingests: the \textbf{Exposure Log}, the \textbf{Narrative Log}, and the \textbf{Night Log}
...
users control to choose which telescope (Simonyi/AuxTel) to focus on, what range of
observation days (dayobs) to request data for. The sidebar [Fig~\ref{fig:sidebar}]
also includes a navigation menu to further detailed pages covered in section~\ref{sec:addpages}.
...
predict how many exposures could be taken each night \cite{Jones2026Scheduler, peter_yoachim_2026_20570788, Naghib19, RTN-016}.
...
displays a timeline of the exposures taken represented as their values of
PSF FWHM (Point Spread Function Full Width at Half-Maximum) and a
...
that we provide in this applet connects to another applet on the page [Fig~\ref{fig:obscondbreak}],
and the zoomed-in selection persists when navigating to other pages in the Nightly Digest for further investigation.
...
or time duration of exposures taken in any number of Science Program categories, observation reason,
...
exposures taken in any number of Science Program categories, observation reason, target name,
...
add slew and overhead detail to exposure data from ConsDB\cite{rubin_nights, Jones2026Scheduler, RTN-016}.
...
The Visit Map applet [Fig~\ref{fig:vma}] shows an interactive Lambert azimuthal equal area projection of a full observation
...
currently they share a one-to-one relationship.}, with controls that let it simulate a planisphere.
We get this RA and Dec data from ConsDB for each exposure.
This data is augmented with almanac data to display environmental
...
other applets were distributed across the sky for that period. This data visualization is
...
and integration of this plot \cite{Jones2026Scheduler, RTN-016}.
...
evaluate survey progress. With their help, we offer a larger view of the visits on an
azimuthal projection (with controls to simulate a planisphere) and is also visualized with an
orthographic projection adjacent (with controls to simulate an armillary sphere).
...
populating these plots is accessed via ConsDB and augmented with almanac data. These
...
based and serves as another effective use of a timeline.

Timelines have become increasingly popular with our users and we continue to arrange
our visualizations to include them in more pages that can leverage a timeline-based display,
...
so that the Nightly Digest may be used as a launching pad towards more detailed debugging tools.
...
many ideas are developed while only some make it into the application.}
   \end{figure}
